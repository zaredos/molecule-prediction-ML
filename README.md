...
○ The code iterates through each dataset in datasets, and then through each
graph within these datasets.
...
○ The computed descriptors are converted into a PyTorch tensor with
torch.float32 data type.
○ These new features are then assigned to the descriptors attribute of each
graph for later use in models.
